---
title: "『BOLT #2: Peer Protocol for Channel Management』の翻訳"
emoji: ""
type: "tech" # tech: 技術記事 / idea: アイデア
topics: ["lightning", "bolt"]
published: false
---

# はじめに

以下の仕様を[Gooele翻訳](https://translate.google.com/)や[DeepL](https://www.deepl.com/translator)で翻訳してみます。
...
Some messages use a `channel_id` to identify the channel.
It's derived from the funding transaction by combining the `funding_txid` and the `funding_output_index`, using big-endian exclusive-OR (i.e. `funding_output_index` alters the last 2 bytes).

Prior to channel establishment, a `temporary_channel_id` is used, which is a random nonce.

Note that as duplicate `temporary_channel_id`s may exist from different peers, APIs which reference channels by their channel id before the funding transaction is created are inherently unsafe.
The only protocol-provided identifier for a channel before funding_created has been exchanged is the (source_node_id, destination_node_id, temporary_channel_id) tuple.
Note that any such APIs which reference channels by their channel id before the funding transaction is confirmed are also not persistent - until you know the script pubkey corresponding to the funding output nothing prevents duplicative channel ids.

</details>

メッセージによっては、チャネルを識別するために `channel_id` を使用するものがあります。
これは、`funding_txid`と`funding_output_index`をビッグエンディアンの排他的論理和で結合することにより、資金取引（funding transaction）から得られます。（つまり、`funding_output_index`では最後の2バイトが変更されます。）

チャネル確立の前に、ランダムなナンスである`temporary_channel_id`が使用されます。

重複した `temporary_channel_id` が異なるピアから存在する可能性があるため、資金調達トランザクションが作成される前にチャネル ID でチャネルを参照する API は本質的に安全ではないことに注意してください。
funding_created が交換される前にプロトコルが提供するチャネルの唯一の識別子は、(source_node_id, destination_node_id, temporary_channel_id) タプルです。
資金調達トランザクションが確認される前にチャネル ID でチャネルを参照する API も永続的ではないことに注意してください。資金調達出力に対応するスクリプト pubkey を知るまでは、チャネル ID の重複を防ぐことはできません。

## チャネル確立（Channel Establishment）

After authenticating and initializing a connection ([BOLT #8](08-transport.md) and [BOLT #1](01-messaging.md#the-init-message), respectively), channel establishment may begin.
This consists of the funding node (funder) sending an `open_channel` message, followed by the responding node (fundee) sending `accept_channel`.
With the channel parameters locked in, the funder is able to create the funding transaction and both versions of the commitment transaction, as described in [BOLT #3](03-transactions.md#bolt-3-bitcoin-transaction-and-script-formats).
The funder then sends the outpoint of the funding output with the `funding_created` message, along with the signature for the fundee's version of the commitment transaction.
Once the fundee learns the funding outpoint, it's able to generate the signature for the funder's version of the commitment transaction and send it over using the `funding_signed` message.

接続 ([BOLT #8](08-transport.md) および [BOLT #1](01-messaging.md#the-init-message)) を認証および初期化した後、チャネルの確立を開始できます。
これは、`open_channel` メッセージを送信する資金提供ノード (資金提供者) と、それに続く`accept_channel` を送信する応答ノード (資金提供者) で構成されます。
[BOLT #3](03-transactions.md#bolt-3-bitcoin-transaction-and-script-formats) で説明されているように、チャネル パラメータがロックインされているため、資金提供者は資金調達トランザクションとコミットメント トランザクションの両方のバージョンを作成できます。
次に、資金提供者は、資金提供者のバージョンのコミットメント トランザクションの署名とともに、`funding_created` メッセージを使用して資金調達出力のアウトポイントを送信します。
資金提供者が資金調達のアウトポイントを知ると、資金提供者のバージョンのコミットメント トランザクションの署名を生成し、それを `funding_signed` メッセージを使用して送信できます。

Once the channel funder receives the `funding_signed` message, it must broadcast the funding transaction to the Bitcoin network.
After the `funding_signed` message is sent/received, both sides should wait for the funding transaction to enter the blockchain and reach the specified depth (number of confirmations). After both sides have sent the `channel_ready` message, the channel is established and can begin normal operation.
The `channel_ready` message includes information that will be used to construct channel authentication proofs.

チャネルの資金提供者が `funding_signed` メッセージを受信したら、資金調達トランザクションをビットコイン ネットワークにブロードキャストする必要があります。
`funding_signed` メッセージが送受信された後、双方は資金調達トランザクションがブロックチェーンに入り、指定された深さ (確認数) に達するまで待機する必要があります。 両側が `channel_ready` メッセージを送信した後、チャネルが確立され、通常の操作を開始できます。
`channel_ready` メッセージには、チャネル認証証明の構築に使用される情報が含まれています。
...
これがいずれかの段階で失敗した場合、または一方のノードが他方のノードによって提供されたチャネル条件が適切でないと判断した場合、チャネルの確立は失敗します。
...
すべてのチャネル メッセージは `temporary_channel_id` (資金調達トランザクションが作成される前) または `channel_id` (資金調達トランザクションから派生) のいずれかによって識別されるため、複数のチャネルが並行して動作できることに注意してください。
...
このメッセージには、ノードに関する情報が含まれており、新しいチャネルをセットアップする必要があることを示しています。
これは、資金調達トランザクションとコミットメント トランザクションの両方のバージョンを作成するための最初のステップです。
...
This is usually the genesis hash of the respective blockchain.
...
`chain_hash` 値は、開かれたチャネルが存在する正確なブロックチェーンを示します。
これは通常、それぞれのブロックチェーンのジェネシス ハッシュです。
`chain_hash` の存在により、ノードは多くの異なるブロックチェーンにわたってチャネルを開くことができ、複数のブロックチェーン内のチャネルを同じピアに対して開くことができます (ターゲット チェーンをサポートしている場合)。
...
`temporary_channel_id` は、資金調達取引が成立するまで、このチャネルをピアごとに識別するために使用され、その時点で、資金調達取引から得られる `channel_id` に置き換えられます。
